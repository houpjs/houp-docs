...
In this tutorial, we'll create a simple `Counter` component, which contains two components: `Label` and `Updater`. 
...
You've probably discovered the problem. Every time you click the button to change the number, 
the number in the another `Counter` component will also change. 
Since we use `useStore(useCounter)` to manage the state, it gets shared across the entire application, 
but this is not what we want. We want the states of the `Counter` components to be independent of each other.
...
`createProvider` function creates a `Provider` component with an array of hooks as its parameter, 
which will be used in the `Counter` component. We use the `Provider` component as the root of the `Counter` component. 
